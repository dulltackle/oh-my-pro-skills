...
本文件基于早期 skill-creator 脚本改造，负责把单次运行结果汇总成：
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                if not grading_file.exists():
                    print(f"警告：{run_dir} 中缺少 grading.json")
                    continue
                ...
                try:
                    ...
                    print(f"警告：{grading_file} 中存在非法 JSON：{e}")
                    continue
                ...
                for exp in raw_expectations:
                    if "text" not in exp or "passed" not in exp:
                        print(f"警告：{grading_file} 中 expectation 缺少必需字段（text/passed/evidence）：{exp}")
...
def main():
    parser = argparse.ArgumentParser(
        description="聚合 benchmark 运行结果并生成汇总"
    )
    parser.add_argument(
        "benchmark_dir",
        type=Path,
    ...
    parser.add_argument(
        "--skill-path",
        default="",
        help="被评测 skill 的目录路径"
    )
    parser.add_argument(
        "--output", "-o",
        type=Path,
        help="benchmark.json 输出路径，默认写入 benchmark 目录"
    )
